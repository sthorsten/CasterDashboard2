    ...
    <b-row v-if="$fetchState.pending">
      <b-col cols="12" md="6">
        <CustomCard
          ...
          outline
          divider
          :title="$t('matches.create_match_group.create')"
        >
          <template #card-body>
      ...
      <b-col cols="12" md="6">
        <CustomCard
          ...
          outline
          divider
          :title="$t('matches.create_match_group.current')"
        >
          <template #card-body>
            <b-skeleton-table :columns="1" :rows="8"></b-skeleton-table>
          </template>
          ...
          <b-col>
            ...
            >
              <template #card-body>
                ...
                <b-form-input
                  class="mb-2"
                  v-model="newMatchGroupName"
                  :placeholder="$t('matches.create_match_group.set_name')"
                />

                <label
                  >{{ $t("matches.create_match_group.select_match") }}:</label
                >

                <v-select
                  class="w-100 mb-4"
                  v-model="newMatchGroupMatches"
                  :options="matches"
                  :multiple="true"
                  :placeholder="$t('matches.create_match_group.select_match')"
                >
                  <template
                    #selected-option="{team_blue_name, team_orange_name, id}"
                  >
                    {{ team_blue_name }} vs. {{ team_orange_name }} (ID:
                    {{ id }})
                  </template>
        ...
        <b-row v-if="matchGroup">
          <b-col>
            <CustomCard
              ...
              outline
              divider
              :title="$t('matches.create_match_group.edit')"
            >
              <template #card-body>
                <label>{{ $t("generic.name") }}</label>
          ...
          <template #card-body>
            ...
            <b-table-simple table-variant="dark" striped small responsive>
              ...
              <b-tr>
                ...
                <b-td class="text-right">
                  <template v-if="matchGroupData.length === 0"> - </template>
                  <template v-for="match in matchGroupData">
                    {{ match.team_blue_name }} vs.
                    {{ match.team_orange_name }} (ID: {{ match.id }})<br />
                  </template>
                </b-td>
              </b-tr>
            </b-table-simple>
...
<script>
  ...
  export default {
    ...
    methods: {
      createNewMatchGroup() {
        if (!this.newMatchGroupName) {
          this.$toast.error(
            this.$t("matches.create_match_group.toasts.no_name")
          )
          return
        }
        ...
        this.$axios
          ...
          .then((data) => {
            this.$toast.success(
              this.$t("matches.create_match_group.toasts.created"),
          ...
          .catch((error) => {
            console.log(error)
            this.$toast.error(
              this.$t("matches.create_match_group.toasts.created_failed"),
      ...
      editMatchGroup() {
        ...
        this.$axios
          .$put(`/api/match_groups/${this.matchGroup.id}/`, {
            users: [this.userID],
          ...
          .then((data) => {
            this.$toast.success(
              this.$t("matches.create_match_group.toasts.edited"),
              this.$t("generic.success")
            )
            this.$fetch()
          })
          .catch((error) => {
            console.log(error)
            this.$toast.error(
              this.$t("matches.create_match_group.toasts.edited_failed"),
              this.$t("generic.error")
            )
          })
      },
      setMatchGroupToOverlay() {
        this.$axios
          .$patch(`/api/overlay/match_data/${this.userID}/`, {
            match_group: this.matchGroup.id,
          })
          .then((data) => {
            this.$toast.success(
              this.$t("matches.create_match_group.toasts.overlay_set"),
              this.$t("generic.success")
            )
          })
          .catch((error) => {
            console.log(error)
            this.$toast.error(
              this.$t("matches.create_match_group.toasts.overlay_set_failed"),
              this.$t("generic.error")
            )
          })
      },
    ...
    mounted() {
      this.$store.commit(
        "setPageTitle",
        this.$t("navigation.create_match_group")
      )
    ...
    mixins: [MatchData, MatchGroupWebsocket],
