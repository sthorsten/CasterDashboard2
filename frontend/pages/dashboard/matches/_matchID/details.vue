          ...
          <template #card-body>
            <b-row class="text-center">
              ...
              <b-col v-for="(map, index) in matchMaps" :key="index">
                <!-- Map image -->
                <template v-if="map.type === 2 || map.type === 3">
                  <div class="img-container pick mb-1">
                    <img
                      class="w-100"
                      :src="mapImgURLs[map.map_name]"
                      alt="-"
                    />
                  </div>
                </template>
                <template v-else-if="map.type === 1 || map.type === 4">
                ...
                <!-- Text & Badge -->
                <span class="text-bold">{{ map.map_name }}</span
                ><br />

                <template v-if="map.type === 1">
                  <b-badge pill variant="danger"> Banned </b-badge>
                  <br />
                ...
                <template v-if="map.type === 2">
                  <b-badge pill variant="success"> Picked </b-badge>
                  <br />
                  <span class="font-italic">
                    {{ map.choose_team_name }}
                  </span>
                  ...
                  <b-row>
                    ...
                    >
                      <template v-if="data.opBans.length > index">
                        <img
                          :src="
                            require(`@/assets/img/operators/${data.opBans[index].operator}.svg`)
                          "
                          style="height: 50px; width: 50px"
                        /><br />
                        <span>{{ data.opBans[index].operator_name }}</span
                        ><br />
                        <span class="font-italic">{{
                          data.opBans[index].team_name
                        }}</span>
                      </template>
                ...
                <b-col>
                  ...
                  >
                    ...
                    <template #cell(score)="table_data">
                      <span
                        >{{ table_data.item.score_blue }} -
                        {{ table_data.item.score_orange }}</span
                      >
                    </template>
                    ...
                    <template #cell(notes)="table_data">
                      <div v-if="table_data.item.notes">
                        <div
                          v-for="(line, index) in table_data.item.notes.split(
                            '\\n'
                          )"
                          :key="index"
                        >
                ...
                <b-col>
                  <b-row class="text-center">
                    ...
                    <b-col cols="12" md="6" lg="3" xl="3" class="mb-2">
                      ...
                      <apexchart
                        type="donut"
                        :options="getBombSpotPickDataOptions(data.map.map)"
                  ...
                  <b-row>
                    ...
                    >
                      <template v-if="bannedOperators.length > index">
                        <img
                          v-if="
                            bannedOperators[index].operator_name === '_none'
                          "
                          :src="
                        ...
                        <br />
                        <span>{{
                          bannedOperators[index].operator_display_name
                        }}</span
                        ><br />
                        <span class="font-italic">{{
  ...
  export default {
    ...
    watch: {
      match: {
        ...
        handler(newState, oldState) {
          ...
          if (
            newState.score_blue >= oldState.score_blue ||
            newState.score_orange >= oldState.score_orange
          ) {
            location.reload()
          }
        },
      ...
      currentMap: {
        ...
        async handler(newState, oldState) {
          if (oldState == null || newState == null || newState.status !== 2)
            return
    ...
    methods: {
      ...
      getBombSpotPickDataOptions(map) {
        ...
        let options = Object.assign({}, this.chartOptions)
        options["labels"] = [
          this.getBombSpotName(mapBombSpots[0].id).name,
          this.getBombSpotName(mapBombSpots[1].id).name,
          this.getBombSpotName(mapBombSpots[2].id).name,
          this.getBombSpotName(mapBombSpots[3].id).name,
        ]
        return options
      },
      ...
      getOfData(rounds) {
        if (rounds == null) return [0, 0]
        let ofBlue = rounds.filter((r) => r.of_team === this.match.team_blue)
          .length
        let ofOrange = rounds.filter(
      ...
      getBombSpotPickData(rounds, map) {
        if (rounds == null) return [0, 0, 0, 0]
        let mapBombSpots = this.bombSpots.filter((bs) => bs.map === map)
        let bs1 = rounds.filter((r) => r.bomb_spot === mapBombSpots[0].id)
          .length
        let bs2 = rounds.filter((r) => r.bomb_spot === mapBombSpots[1].id)
          .length
        let bs3 = rounds.filter((r) => r.bomb_spot === mapBombSpots[2].id)
          .length
        let bs4 = rounds.filter((r) => r.bomb_spot === mapBombSpots[3].id)
          .length
