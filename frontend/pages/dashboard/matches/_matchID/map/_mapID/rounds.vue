                            ...
                            <b-col lg="6">
                                ...
                                <div v-for="bs in bombSpots" :key="bs.id">
                                    <b-btn v-if="selectedBombSpot === bs" :disabled="mapLocked"
                                        ...
                                        <b-badge variant="light" pill class="mr-1">{{ bs.floor }}</b-badge>
                                    ...
                                    <b-btn v-else :disabled="mapLocked"
                                        ...
                                        <b-badge variant="primary" pill class="mr-1">{{ bs.floor }}</b-badge>
                                        <span class="sl-only">{{ bs.name }}</span>
                                    </b-btn>
                                </div>
                    ...
                    <template #card-body>
                        ...
                        <b-table table-variant="dark" small striped sort-icon-left responsive :items="rounds"
                            ...
                            <template #cell(bomb_spot)="data">
                                ...
                                <span class="sl-only">{{ getBombSpotName(data.item.bomb_spot).name }}</span>
                            </template>
                            ...
                            <template #cell(notes)="data">
                                <div v-if="data.item.notes">
                                    <div v-for="(line, index) in data.item.notes.split('\\n')" :key="index">
                                        <span>{{ line }}</span><br>
                                    </div>
                                </div>
                                <span v-else>-</span>
...
export default {
    ...
    computed: {
        ...
        bombSpotPickDataOptions() {
            let options = Object.assign({}, this.chartOptions)
            options['labels'] = [this.getBombSpotName(this.bombSpots[0].id).name, this.getBombSpotName(this.bombSpots[1].id).name,
                this.getBombSpotName(this.bombSpots[2].id).name, this.getBombSpotName(this.bombSpots[3].id).name]
            return options
        },
    ...
    methods: {
        ...
        addRound() {
            ...
            this.$axios.$post('/api/matches/round/', data
            ).then((response) => {
                this.resetRoundData()
                this.$toast.success(this.$t('matches.rounds.toasts.round_added'), this.$t('generic.success'), {timeout: 1000})
                this.loadingSmall = {}
            }).catch(() => {
        ...
        removeLastRound() {
            ...
            this.$axios.$delete(`/api/matches/round/${lastRound.id}/`
            ).then(() => {
        ...
        finishMap() {
            this.$bvModal.msgBoxConfirm(this.$t('matches.rounds.finish_map_confirm_1') + " " + this.$t('matches.rounds.finish_map_confirm_2'), {
                ...
                if (value) {
                    ...
                    this.$axios.$patch(`/api/matches/maps/${this.matchMap.id}/`, data
                    ).then(() => {

                        this.$axios.$get(`/api/match/${this.matchID}/`
                        ).then((matchData) => {
                            let nextURL = "";
                            ...
                            if (nextURL === "overview") {
                                ...
                                this.$router.push(`/dashboard/matches/${this.matchID}/overview`)

                            } else if (nextURL === "nextMap") {
                                // Redirect to next map
                                this.$axios.$get(`/api/matches/maps/?match=${this.matchID}`
                                ).then((data) => {
                                    this.$toast.info(this.$t('matches.rounds.map_finished'))
                                    let next_map_id = data.filter(m => (m.play_order === this.matchMap.play_order + 1))[0].map
                                    this.$router.push(`/dashboard/matches/${this.matchID}/map/${next_map_id}/opbans`)
                                })
                            }
                        })
    ...
    mounted() {
        ...
        this.$store.commit("setBreadcrumbPath", ["Dashboard", "Matches", this.$route.params.matchID, "Map", this.$route.params.mapID, "Rounds"])
    },
