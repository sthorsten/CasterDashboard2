    ...
    <b-row v-else>
      ...
      <b-col md="5">
        <CustomCard
          ...
          outline
          divider
          :title="$t('matches.maps.settings')"
        >
          <template #card-body>
            <b-form-group :label="$t('matches.maps.select_map_pool')">
              <v-select
                class="w-100"
                v-model="mapPoolSelected"
                :options="mapPools"
                label="name"
              />
            </b-form-group>
            ...
            <b-btn
              variant="danger"
              class="btn-block"
              :disabled="mapsLocked || matchMaps.length === 0"
              @click="removeLastMap"
            >
              <b-spinner
                v-if="loadingSmall.removeLastMap"
                variant="light"
                small
              />
              <span v-else>
            ...
            >
              <b-spinner
                v-if="loadingSmall.removeAllMaps"
                variant="light"
                small
              />
              <span v-else>
            ...
            <template v-if="nextMap">
              <nuxt-link
                :to="`/dashboard/matches/${matchID}/map/${nextMap.map}/opbans`"
              >
        ...
        <CustomCard color="secondary" outline divider title="Log">
          <template #card-body>
            <div class="font-italic mb-2">
              {{ $t("matches.maps.match_is_best_of") }}
              <b>Best Of {{ match.best_of }}</b
              >!
            </div>
              ...
              <li v-for="map in matchMaps" :key="map.id">
                ...
                <template v-else-if="map.type === 3">
                  <span class="text-success text-bold">
                    ...
                    {{ map.map_name }} (Map #{{ map.play_order }})
                  </span>
                </template>
                ...
                <template v-else>
                  ...
                  <template v-if="map.type !== 1">
                    <span class="text-success text-bold">
                      {{ map.order }} / {{ map.type_name }} /
                      {{ map.choose_team_name }} / {{ map.map_name }} (Map #{{
                        map.play_order
                      }})
                    </span>
                  </template>
                ...
                <b-row align-v="center" class="text-center">
                  ...
                  <b-col cols="2" class="align-items-center">
                    <template v-if="matchMapFiltered[map.id - 1] != null">
                      <template
                        v-if="
                          matchMapFiltered[map.id - 1].type === 2 ||
                          matchMapFiltered[map.id - 1].type === 3
                        "
                      >
                        <div class="img-container pick">
                          <img
                            class="w-100"
                            :src="mapImgURLs[map['name']]"
                            alt="-"
                          />
                        </div>
                      </template>
                      <template
                        v-else-if="
                          matchMapFiltered[map.id - 1].type === 1 ||
                          matchMapFiltered[map.id - 1].type === 4
                        "
                      >
                  ...
                  <b-col cols="3">
                    <b-btn
                      variant="danger"
                      class="btn-block mb-0"
                      :disabled="mapsLocked || matchMaps.length === 7"
                      @click="selectMap(map.id, 'ban', match.team_blue)"
                    >
                      <b-spinner
                        v-if="
                          loadingSmall.mapLoading &&
                          loadingSmall.ban &&
                          loadingSmall.map === map.id &&
                          loadingSmall.team === match.team_blue
                        "
                        variant="light"
                        small
                      />
                  ...
                  <!-- Data -->
                  <b-col cols="4">
                    <span class="text-bold">{{ map.name }}</span
                    ><br />
                    ...
                    >
                      <b-spinner
                        v-if="
                          ...
                          loadingSmall.ban &&
                          loadingSmall.map === map.id &&
                          loadingSmall.team === match.team_orange
                        "
                        variant="light"
                        small
                      />
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      nextMap() {
        let play_maps = this.matchMaps.filter(
          (m) => m.play_order > 0 && m.status < 3
        )
    ...
    watch: {
      resetAllCounter: function (newState) {
        if (newState === 0) {
          ...
          this.$toast.success(
            this.$t("matches.maps.toasts.maps_removed"),
    ...
    async fetch() {
      ...
      this.mapPoolSelected = this.mapPools.filter(
        (e) => e.name === "Competitive"
      )[0]
    ...
    methods: {
      selectMap(map, type, team) {
        ...
        if (duplicates.length > 0) {
          this.$toast.warning(
            this.$t("matches.maps.toasts.map_already_selected")
          )
          return
        }
        ...
        this.$axios
          .$post("/api/matches/maps/", data)
          .then(() => {
            this.$toast.success(
              this.$t("matches.maps.toasts.map_added"),
              this.$t("generic.success"),
              {timeout: 2000}
            )
          })
          .catch((error) => {
            ...
            this.$toast.error(
              this.$t("matches.maps.toasts.map_added_failed"),
      ...
      removeLastMap() {
        ...
        let last_map_id = this.matchMaps.filter(
          (m) => m.order === this.matchMaps.length
        )[0].id

        this.$axios
          .$delete(`/api/matches/maps/${last_map_id}/`)
          .then(() => {
            this.$toast.success(
              this.$t("matches.maps.toasts.map_removed"),
              this.$t("generic.success"),
              {timeout: 2000}
            )
          })
          .catch((error) => {
            ...
            this.$toast.error(
              this.$t("matches.maps.toasts.map_removed_failed"),
              this.$t("generic.error")
            )
          })
    ...
    mixins: [SingleMatch, Maps, MapPools, MatchMapAllWebsocket],
