<template>
  <div>
    <CustomCard
      ...
      outline
      divider
      :title="$t('matches.overview.match_info')"
    ...
    <b-row v-if="!this.$fetchState.pending">
              ...
              <b-tbody>
                ...
                <b-tr>
                  <b-th class="text-bold">{{
                    $tc("generic.user", match.user.length)
                  }}</b-th>
                  <b-td class="text-right">{{
                    match.user_name.join(", ")
                  }}</b-td>
                </b-tr>
                ...
                <b-tr class="bg-primary">
                  <b-th colspan="2">{{
                    $t("matches.overview.match_details")
                  }}</b-th>
                </b-tr>
                ...
                <b-tr>
                  <b-th class="text-bold">{{
                    $tc("core.sponsor", match.sponsors.length)
                  }}</b-th>
                ...
                <b-tr class="bg-primary">
                  <b-th colspan="2">{{
                    $t("matches.overview.match_results")
                  }}</b-th>
                </b-tr>

                <b-tr>
                  <b-th class="text-bold">{{
                    $t("matches.overview.current_score")
                  }}</b-th>
                  <b-td class="text-right"
                    >{{ match.score_blue }} - {{ match.score_orange }}</b-td
                  >
                </b-tr>

                <b-tr>
                  <b-th class="text-bold">{{
                    $t("matches.overview.win_team")
                  }}</b-th>
                  <b-td v-if="match.win_team_name" class="text-right">{{
                    match.win_team_name
                  }}</b-td>
                  <b-td v-else class="text-right">-</b-td>
                </b-tr>
      ...
      <b-col md="6">
        <CustomCard
          ...
          outline
          divider
          :title="$t('matches.overview.actions.title')"
        >
          <template #card-body>
            <b-row>
      ...
      <ShareMatchModal
        :show="shareMatchModal"
        @hide="shareMatchModal = false"
        :match="match"
        @reload="
          $fetch()
  ...
  export default {
    ...
    methods: {
      setMatchToOverlay(next) {
        ...
        this.$axios
          .$put(`/api/overlay/match_data/${this.$auth.user.id}/`, data)
          .then((data) => {
            this.$toast.success(
              this.$t("matches.overview.toasts.overlay_success")
            )
          })
          .catch((error) => {
            console.log(error)
