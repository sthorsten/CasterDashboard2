<template>
  <b-modal
    ...
    :visible="show"
    size="lg"
    :title="this.$t('matches.overview.modal.share_title')"
    hide-footer
  ...
  >
    <b-form @submit.prevent="onSubmit" novalidate>
      <b-form-group
        class="mb-3"
        :label="this.$t('matches.overview.modal.share_user')"
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit() {
        ...
        // Update match
        this.$axios
          .$patch(`/api/match/${this.match.id}/`, this.match)
          .then(() => {
            this.$toast.success(
              this.$t("matches.overview.modal.toasts.success")
            )
            this.$emit("reload")
          })
          .catch((error) => {
    ...
    async fetch() {
      this.$axios.$get("/api/user/").then((data) => {
        ...
        let allUsers = data.filter(
          (u) => u.id !== this.$auth.user.id && u.id !== 1
        )
