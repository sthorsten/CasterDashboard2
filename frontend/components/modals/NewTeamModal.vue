<template>
  <b-modal
    :visible="show"
    size="lg"
    :title="this.$t('data.teams.add_new_team_btn')"
    hide-footer
  ...
  >
    <b-form @submit.prevent="onSubmit" novalidate>
      <b-form-group
        class="mb-3"
        :label="this.$t('data.teams.modal.team_name')"
      ...
      >
        <b-input-group>
          <b-form-input
            ...
            type="text"
            :placeholder="this.$t('data.teams.modal.team_name_text')"
            aria-describedby="new-team-name-feedback"
            maxlength="22"
            :state="new_team_name_state"
      ...
      <b-form-group
        class="mb-3"
        :label="this.$t('data.teams.modal.logo_upload')"
        label-for="new-team-file"
        label-cols-sm="4"
        label-cols-lg="3"
      >
        <b-input-group>
          <b-form-file
            id="new-team-file"
            v-model="new_team_logo_file"
            :placeholder="this.$t('data.teams.modal.logo_upload_text')"
            :drop-placeholder="
              this.$t('data.teams.modal.logo_upload_text_drop')
            "
      ...
      <b-form-group
        class="mb-3"
        :label="this.$t('data.teams.modal.logo_url')"
        label-for="new-team-logo-url"
        label-cols-sm="4"
        label-cols-lg="3"
      >
        <b-input-group>
          <b-form-input
            id="new-team-logo-url"
            v-model="new_team_logo_url"
            type="url"
    ...
    <b-row>
      <b-col>
        <p>
          ...
          <b class="text-uppercase">{{ this.$t("generic.quick_info") }}</b
          ><br />
        </p>
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit() {
        // Validate name
        this.new_team_name_state = Boolean(this.new_team_name)
        if (this.new_team_name_state) this.addNewTeam()
      },
      addNewTeam() {
        this.$emit("hide")
        this.$emit("loading")
        ...
        this.$axios
          ...
          .catch((error) => {
            ...
            if (error.response.status === 500)
              this.$toast.error(
                this.$t("generic.internal_server_error"),
                this.$t("generic.oops")
              )
            else
              this.$toast.error(
                error.response.data.error.toString(),
                this.$t("data.teams.modal.team_create_failed")
              )
            this.$emit("reload")
          })
