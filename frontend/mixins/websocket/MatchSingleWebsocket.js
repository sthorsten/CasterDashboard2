  ...
  methods: {
    connectMatchSingleWebsocket() {
      return new Promise((resolve, reject) => {
        ...
        let websocketURL = `${WebsocketURL.getURL(this.$config)}/ws/match/${
          this.matchID
        }/`
        ...
        this.matchSingleWebsocket.onclose = (e) => {
          ...
          console.warn(
            `MatchSingle websocket connection closed unexpectedly. Trying to reconnect (${timeout}s.)`
          )
          this.matchSingleWebsocketStatus = WebsocketStatus.RECONNECTING
          this.matchSingleWebsocketTimeout = setTimeout(
