  ...
  <b-input-group>
    ...
    <b-input-group-append>
      ...
      <b-input-group-text class="bg-warning">
        <a
          :href="`${baseLink}/overlays/${$auth.user.username}/${url}?layer-name=${title}&layer-width=1920&layer-height=1080`"
      ...
      <b-input-group-text class="bg-success">
        <a
          :href="`${baseLink}/overlays/${$auth.user.username}/${url}`"
          target="_blank"
...
<script>
  export default {
    ...
    methods: {
      overlayCopied() {
        this.$toast.success(
          this.$t("overlays.customize.overlay_copied"),
