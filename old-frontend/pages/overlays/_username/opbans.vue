  ...
  <div v-if="!$fetchState.pending">
    <div v-if="animMain" class="main">
      ...
      >
        <div
          v-if="
            (animOps >= 1 && getBanOrder(1) === 2) ||
            (animOps >= 0 && getBanOrder(1) === 1)
          "
          id="op1"
          class="opban-container"
        >
      ...
      >
        <div
          v-if="
            (animOps >= 2 && getBanOrder(2) === 3) ||
            (animOps >= 3 && getBanOrder(2) === 4)
          "
          id="op2"
          class="opban-container"
        >
      ...
      >
        <div
          v-if="
            (animOps >= 3 && getBanOrder(3) === 4) ||
            (animOps >= 2 && getBanOrder(3) === 3)
          "
          id="op3"
          class="opban-container"
        >
      ...
      >
        <div
          v-if="
            (animOps >= 0 && getBanOrder(4) === 1) ||
            (animOps >= 1 && getBanOrder(4) === 2)
          "
          id="op4"
          class="opban-container"
        >
  ...
  export default {
    ...
    methods: {
      ...
      getOperatorImgURL(team, side) {
        ...
        let operator = this.bannedOperators.filter(
          (o) => o.operator_side === side && o.team === team
        )[0]
        if (operator) {
          if (operator.operator_name === "_none") {
            return `<img class="opban-image" src="${require("@/assets/img/operators/" +
              operator.operator +
              ".svg")}" alt="">`
          }
          return this.operatorIcons[operator.operator_name].toSVG({
            class: "opban-image",
          })
        }
        return 0
