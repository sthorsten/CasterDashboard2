    ...
    <b-row v-else>
                ...
                <b-row>
                  <b-col xl="6">
                    <template
                      v-if="
                        matchMap.atk_team &&
                        matchMap.atk_team === match.team_blue
                      "
                    >
                      <b-btn
                        variant="primary"
                    ...
                    <template v-else>
                      ...
                      >
                        {{ match.team_blue_name }}
                        <b-spinner
                          v-if="loadingSmall.atkTeam === match.team_blue"
                          variant="light"
                          small
                        />
                      </b-btn>
                  ...
                  <b-col xl="6">
                    ...
                    <template v-else>
                      ...
                      >
                        {{ match.team_orange_name }}
                        <b-spinner
                          v-if="loadingSmall.atkTeam === match.team_orange"
                          variant="light"
                          small
                        />
                      </b-btn>
                  ...
                  <b-col xl="6">
                    ...
                    <template v-else>
                      ...
                      >
                        {{ match.team_blue_name }}
                        <b-spinner
                          v-if="loadingSmall.otAtkTeam === match.team_blue"
                          variant="light"
                          small
                        />
                      </b-btn>
                  ...
                  <b-col xl="6">
                    ...
                    <template v-else>
                      ...
                      >
                        {{ match.team_orange_name }}
                        <b-spinner
                          v-if="loadingSmall.otAtkTeam === match.team_orange"
                          variant="light"
                          small
                        />
                      </b-btn>
            ...
            <b-row align-v="center">
              ...
              >
                <template v-if="bannedOperators.length > index">
                  <img
                    v-if="bannedOperators[index].operator_name === '_none'"
                    :src="
                      require(`@/assets/img/operators/${bannedOperators[index].operator}.svg`)
                    "
                  ...
                  <svg
                    ...
                    v-html="
                      operatorIcons[
                        bannedOperators[index].operator_name
                      ].toSVG()
                    "
                    style="height: 50px; width: 50px"
                  />
                  <br />
                  <span>{{ bannedOperators[index].operator_display_name }}</span
                  ><br />
                  <span class="font-italic">{{
                    bannedOperators[index].team_name
                  }}</span>
                </template>
            ...
            <b-row>
              <b-col lg="6">
                ...
                >
                  ...
                  <b-spinner
                    v-if="loadingSmall === 'remove-last-opban'"
                    variant="light"
                    small
                  />
                </b-btn>
              ...
              <b-col lg="6" class="mt-2 mt-lg-0">
                ...
                >
                  ...
                  <b-spinner
                    v-if="loadingSmall === 'remove-all-opBans'"
                    variant="light"
                    small
                  />
                </b-btn>
      ...
      <b-col lg="6">
        <CustomCard
          ...
          outline
          divider
          :title="$t('matches.op_bans.ban_operators')"
        >
          <template #card-body>
            <b-row>
              <!-- ATK Ops -->
              <b-col md="6" class="text-center">
                <label>{{ $t("matches.op_bans.atk_ops") }}</label
                ><br />
                ...
                <b-row class="mt-2">
                  <b-col cols="6" v-for="op in atkOps" :key="op.id">
                    <template v-if="isOperatorBanned(op.id)">
                      <b-btn
                        variant="danger"
                        class="btn-block mb-2 text-left"
                        disabled
                      >
                        <img
                          v-if="op.name === '_none'"
                        ...
                        <svg
                          v-else
                          v-html="operatorIcons[op.name].toSVG()"
                          style="height: 25px; width: 25px"
                        ></svg>
                        {{ op.display_name }}
                      </b-btn>
                    ...
                    <template v-else>
                      <b-btn
                        ...
                        class="btn-block mb-2 text-left"
                        :disabled="
                          atkOpsBanned || !matchMap.atk_team || mapLocked
                        "
              ...
              <!-- DEF Ops -->
              <b-col md="6" class="text-center">
                <label>{{ $t("matches.op_bans.def_ops") }}</label
                ><br />
  ...
  export default {
    ...
    methods: {
      ...
      setATKTeam(team) {
        ...
        this.$axios
          .$patch(`/api/matches/maps/${this.matchMap.id}/`, {
            atk_team: team,
          })
          .then(() => {
          ...
          .catch(() => {
            this.$toast.error(
              this.$t("matches.op_bans.toasts.atk_team_set_failed")
            )
          })
      ...
      setOTATKTeam(team) {
        ...
        this.$axios
          ...
          .then(() => {
            this.$toast.success(
              this.$t("matches.op_bans.toasts.ot_atk_team_set")
            )
          })
          .catch(() => {
            this.$toast.error(
              this.$t("matches.op_bans.toasts.ot_atk_team_set_failed")
            )
          })
          .then(() => (this.loadingSmall = {}))
      },
      swapTeams() {
        ...
        this.$axios
          .$patch(`/api/match/${this.match.id}/`, data)
          .then(() => {
            this.$toast.success(this.$t("matches.op_bans.toasts.teams_swapped"))
          })
          .catch(() => {
            this.$toast.error(
              this.$t("matches.op_bans.toasts.teams_swapped_failed")
            )
          })
          .then(() => {
            this.getSingleMatch()
      ...
      banOperator(id) {
        ...
        this.$axios
          .$post("/api/matches/opbans/", data)
          .then(() => {
            this.$toast.success(this.$t("matches.op_bans.toasts.opban_added"))
          })
          .catch(() => {
            this.$toast.error(
              this.$t("matches.op_bans.toasts.opban_added_failed")
            )
          })
          .then(() => (this.loadingSmall = {}))
      },
      removeLastOperatorBan() {
        ...
        this.$axios
          .$delete(`/api/matches/opbans/${lastBanID}/`)
          .then(() => {
            this.$toast.success(this.$t("matches.op_bans.toasts.opban_removed"))
          })
          .catch(() => {
            this.$toast.error(
              this.$t("matches.op_bans.toasts.opban_removed_failed")
            )
          })
          .then(() => (this.loadingSmall = {}))
      },
      async removeAllOperatorsBans() {
        ...
        await this.bannedOperators.forEach((o) => {
          this.$axios
            .$delete(`/api/matches/opbans/${o.id}/`)
            .then(() => counter--)
            .catch(() => {
              this.$toast.error(
                this.$t("matches.op_bans.toasts.all_opbans_removed_failed")
              )
            })
        })
        ...
        this.loadingSmall = {}
        this.$toast.success(
          this.$t("matches.op_bans.toasts.all_opbans_removed")
        )
      },
    },
