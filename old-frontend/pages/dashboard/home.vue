    ...
    <b-row>
      <b-col>
        <b-alert
          variant="success"
          show=""
          class="text-center bg-gradient-success"
        >
          <span
            ><b>{{ $t("home.welcome") }} </b
            >{{ this.$auth.user.userName }} &#128075;</span
          ><br />
  ...
  export default {
    ...
    async fetch() {
      ...
      this.notifications.forEach((n) => {
        ...
        if (n.valid_until != null && now > valid_until)
          this.notifications.splice(n.index, 1)
      })
    },
